-- HomegoingHQ — Migration v31: header wordmark (text next to the logo) with
-- font / bold / italic / size, for partners whose logo is small or icon-only.
...
create or replace function public.admin_set_header_text(
  target uuid, p_text text, p_font text, p_bold boolean, p_italic boolean, p_size text)
returns boolean language plpgsql security definer set search_path = public as $$
begin
  if not public.is_admin() then return false; end if;
  insert into public.tenant_branding
      (account_id, header_text, header_font, header_bold, header_italic, header_size, updated_at)
  values
      (target, nullif(trim(coalesce(p_text,'')),''), nullif(p_font,''),
       coalesce(p_bold,false), coalesce(p_italic,false), coalesce(nullif(p_size,''),'md'), now())
...
grant execute on function public.admin_set_header_text(uuid,text,text,boolean,boolean,text) to authenticated;

-- Recreate public_branding so branded pages receive the wordmark fields.
...
begin
  ...
  select ca.id, ca.business_name, ca.tier, ca.tenant_type, ca.status,
         ...
         tb.header_text, tb.header_font, tb.header_bold, tb.header_italic, tb.header_size
    into a
